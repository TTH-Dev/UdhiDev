...
const IPurchaseApproval: React.FC = () => {
  ...
  const columns = [
    ...
    //   title: "Action",
    //   dataIndex: "Tax",
    //   render:(_text:any)=>(<><Button className="p-0" style={{width:"40px",height:"40px",backgroundColor:"#3497F9",border:"none"}}><MdEdit style={{color:"#fff",fontSize:"20px"}}/></Button></>)
    // },
  ];
  ...
  const getData = async () => {
    try {
      ...
      const res = await axios.get(
        `${api_url}/api/pharmacy-purchase/filter?status=Requested`,
        {
          headers: {
  ...
  const handleUpdateStatus = async (value: any) => {
    try {
      ...
      await axios.post(
        `${api_url}/api/pharmacy-purchase/bulk-edit`,
...
export default IPurchaseApproval;
