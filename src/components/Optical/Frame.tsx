...
const Frame: React.FC = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_: any, record: any) => (
        <>
          <Link to={`/optical/product/edit-product-frame?id=${record._id}`}>
            <Button
              className="p-0 ms-1"
              style={{
                border: "none",
  ...
  const getProduct = async () => {
    try {
      ...
      const res = await axios.get(
        `${api_url}/api/optical-product?type=Frame&${selectedColumn}=${searchTerm}&limit=${pageSize}&page=${currentPage}`,
        {
          headers: {
  ...
  useEffect(() => {
    if (isAddnew === "addnew") {
      navigate("/optical/product/add-product-frame");
