...
const VendorFrame: React.FC = () => {
  ...
  const getVendor = async () => {
    try {
      ...
      const res = await axios.get(
        `${api_url}/api/optical-vendor?vendorType=Frame&${selectedColumn}=${searchTerm}&limit=${pageSize}&page=${currentPage}`,
        {
          headers: {
  ...
  useEffect(() => {
    if (isAddnew === "addnew") {
      navigate("/optical/vendor/add-vendor-frame");
