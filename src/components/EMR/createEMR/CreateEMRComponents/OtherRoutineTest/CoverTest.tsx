  ...
  const getDatas = async (id: any) => {
    try {
      ...
      const res = await axios.get(
        `${api_url}/api/other-routine-test/get-by-date?section=coverTest&patientId=${id}&enteredDate=${moment(
          new Date()
        ).format("YYYY-MM-DD")}`,
        {
          headers: {
      ...
      const otherTests = res?.data?.data?.coverTest?.results;
      console.log(otherTests,"otherTests");
  ...
  return (
    <>
      <div className="emr-complaints-box mt-4 rounded ps-4">
        <p className="emr-search-text">Cover Test</p>
      </div>
