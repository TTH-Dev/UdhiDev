  ...
  const getDatas = async (id: any) => {
    try {
      ...
      const res = await axios.get(
        `${api_url}/api/other-routine-test/get-by-date?section=nonContactTonoMetry&patientId=${id}&enteredDate=${moment(
          new Date()
        ).format("YYYY-MM-DD")}`,
        {
          headers: {
      ...
      const otherTests = res?.data?.data?.nonContactTonoMetry?.results[0]?.nonContactTonoMetry;
  ...
  return (
    <>
      <div className="emr-complaints-box mt-4 rounded ps-4">
        <p className="emr-search-text">Non Contact Tonometry</p>
      </div>
