            ...
            const getDatas = async (id: any) => {
              try {
                ...
                const res = await axios.get(
                  `${api_url}/api/past-history/getBy-section?section=previousSurgeries&patientId=${id}&enteredDate=${moment(
                    new Date()
                  ).format("YYYY-MM-DD")}`,
                  {
                    headers: {
                ...
                setData(res.data.data.previousSurgeries);
              } catch (error: any) {
                console.log(error);
  ...
  return (
    <>
       <div className="emr-complaints-box mt-4 rounded ps-4">
        <p className="emr-search-text">Previous Surgeries</p>
        </div>
