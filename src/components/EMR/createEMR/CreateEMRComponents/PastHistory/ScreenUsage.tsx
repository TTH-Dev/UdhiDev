...
const ScreenUsage: React.FC = () => {
  const columns = [ "Hours", "Note"];
        ...
        const getDatas = async (id: any) => {
          try {
            ...
            const res = await axios.get(
              `${api_url}/api/past-history/getBy-section?section=screenUsage&patientId=${id}&enteredDate=${moment(
                new Date()
              ).format("YYYY-MM-DD")}`,
              {
                headers: {
            ...
            setData(res.data.data.screenUsage);
          } catch (error: any) {
            console.log(error);
  ...
  return (
    <>
     <div className="emr-complaints-box mt-4 rounded ps-4">
        <p className="emr-search-text">Screen Usage</p>
        </div>
            ...
            <Table>
              ...
              <TableBody>
                {data.length>0&&data.map((row:any, index:any) => (
                  <TableRow key={index}>
                    <TableCell>{row?.booleanBox.toString()||"-"}</TableCell>
