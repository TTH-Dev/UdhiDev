...
const MOcularMovements: React.FC = () => {
  ...
  const handleDownloadExcel = (id: string) => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Ocular Movements");

    XLSX.writeFile(workbook, `Ocular Movements ${id.slice(-5)}.xlsx`);
  };
  ...
  const getDatas = async (id: any) => {
    try {
      ...
      const ocu = routineTests.flatMap(
        (test: any) =>
          test.ocularMovements?.map((entry: any) => ({
...
export default MOcularMovements;
