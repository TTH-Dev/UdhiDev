...
const MCoverTest: React.FC = () => {
  ...
  const getDatas = async (id: any) => {
    try {
      ...
      const cover = routineTests.flatMap(
        (test: any) =>
          test.coverTest?.map((entry: any) => ({
  ...
  const handleDownloadExcel = (id: string) => {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "Cover Test");
  
      XLSX.writeFile(workbook, `Cover Test ${id.slice(-5)}.xlsx`);
    };
