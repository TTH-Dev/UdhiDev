...
const MPreviousSurgeries: React.FC = () => {
  const [data, setData] = useState<Sur[]>([]);
  const [currentPage, setCurrentPage] = useState(1);
  const pageSize = 10;

  const printRef = useRef<HTMLDivElement>(null);
  const columns = ["Entered Date", "Surgeries"];
  ...
  const handleDownloadExcel = (id: string) => {
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, "Previous Surgeries");

    XLSX.writeFile(workbook, `Previous Surgeries ${id.slice(-5)}.xlsx`);
  };
  ...
  const fetchData = async (id: string) => {
    try {
      ...
      const res = await axios.get(
        `${api_url}/api/past-history/getBy-section?section=previousSurgeries&patientId=${id}`,
        {
          headers: {
      ...
      const response = res?.data?.data?.previousSurgeries?.map((sur: any) => ({
        enteredDate: sur.enteredDate
          ? moment(sur.enteredDate).format("DD-MM-YYYY")
          : "-",
        surgeries: sur?.surgeries,
      }));
...
export default MPreviousSurgeries;
