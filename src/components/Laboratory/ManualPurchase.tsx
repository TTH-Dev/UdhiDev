...
const ManualPurchase = () => {
  ...
  const vendorDrop = async () => {
    ...
    try {
      ...
      const dfId = res.data.data.data.map((val: any) => ({
        label: val.vendorName,
        value: val.id,
      }));

      setDropData({ ...dropData, name: dfs, id: dfId });
    } catch (error: any) {
      console.log(error);
  ...
  const getproductDrop = async () => {
    try {
      ...
      const res = await axios.post(
        `${api_url}/api/product/findyBy-vendor`,
        {
  ...
  const handleAdd = () => {
    ...
    let idV: any = dropData.id.find((item: any) => item.label === vendorName);
    const newProduct = {
      vendor: vendorName,
      vendorId: idV?.value || "",
      ...
      quantity: quantity,
      rate: productDetail.productRate,
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.post(`${api_url}/api/purchase`, data, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  useEffect(() => {
    const ls = allProduct.filter((val: any) => val.productName === productName);
    if (ls.length > 0) {
      console.log(ls[0].units);
      setUnit(ls[0].units);
