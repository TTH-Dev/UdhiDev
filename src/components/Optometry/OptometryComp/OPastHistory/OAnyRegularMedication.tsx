  ...
  const getData = async (ids: any) => {
    try {
      ...
      const res = await axios.get(
        `${api_url}/api/past-history/getBy-section/?section=anyRegularMedication&patientId=${ids}&enteredDate=${moment(
          new Date()
        ).format("YYYY-MM-DD")}`,
        {
          headers: {
      ...
      const df = res.data.data.anyRegularMedication[0];
        ...
        <Form form={form} layout="vertical">
          <div className="row">
            <div className="col-lg-6 col-md-6">
              {" "}
              <Form.Item
                label={<span className="emr-label">Medication</span>}
                name="medication"
              >
                <TextArea rows={4} />
              </Form.Item>
