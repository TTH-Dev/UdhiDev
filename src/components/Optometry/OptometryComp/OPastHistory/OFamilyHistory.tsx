    ...
    try {
      ...
      const data = {
        ...
        data: {
          patientId: patientId,
          enteredDate: Date.now,
          systemicDisease: values.systemicDisease,
          ophthalmicDisease: values.ophthalmicDisease,
        },
      };
  ...
  const getData = async (ids: any) => {
    try {
      ...
      const res = await axios.get(
        `${api_url}/api/past-history/getBy-section/?section=familyHistory&patientId=${ids}&enteredDate=${moment(
          new Date()
        ).format("YYYY-MM-DD")}`,
        {
          headers: {
      ...
      const df = res.data.data.familyHistory[0];

      if (df && Object.keys(df).length > 0) {
        setIsUpdate(true);
        setUpdateId(df._id);
        form.setFieldsValue({
          ophthalmicDisease: df.ophthalmicDisease,
          systemicDisease: df.systemicDisease,
        });
      } else {
        setIsUpdate(false);
      }
        ...
        <Form layout="vertical" form={form}>
          <div className="row">
            <div className="col-lg-6 col-md-6">
              {" "}
              <Form.Item
                label={<span className="emr-label">Systemic Disease</span>}
            ...
            <div className="col-lg-6 col-md-6">
              {" "}
              <Form.Item
                label={<span className="emr-label">Ophthalmic Disease</span>}
                name="ophthalmicDisease"
              >
                <TextArea />
              </Form.Item>
            </div>
