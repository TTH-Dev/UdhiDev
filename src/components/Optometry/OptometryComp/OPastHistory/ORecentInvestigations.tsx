  ...
  const handleSave = async () => {
    try {
      ...
      const data = {
        ...
        section: "recentInvestigation",
        data: {
          patientId: patientId,
          enteredDate: Date.now,
          recentInvestigation: values.recentInvestigation,
        },
      };
  ...
  const getData = async (ids: any) => {
    try {
      ...
      const res = await axios.get(
        `${api_url}/api/past-history/getBy-section/?section=recentInvestigation&patientId=${ids}&enteredDate=${moment(
          new Date()
        ).format("YYYY-MM-DD")}`,
        {
          headers: {
      ...
      const df = res.data.data.recentInvestigation[0];

      if (df && Object.keys(df).length > 0) {
        setIsUpdate(true);
        setUpdateId(df._id);
        form.setFieldsValue({
          recentInvestigation: df.recentInvestigation,
        });
      } else {
        setIsUpdate(false);
      }
        ...
        <Form layout="vertical" form={form}>
          <div className="row">
            <div className="col-lg-6 col-md-6">
              {" "}
              <Form.Item
                label={<span className="emr-label">Recent Investigation</span>}
                name="recentInvestigation"
              >
                <TextArea rows={4} />
              </Form.Item>
            </div>
