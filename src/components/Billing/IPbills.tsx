...
const IPbills: React.FC = () => {
  ...
  const getBillingData = async () => {
    ...
    try {
      const res = await axios.get(`${api_url}/api/billing/filter?billFor=ipBilling&UHID=${filervalue.uuhid}&phoneNo=${filervalue.phoeNo}&status=${filervalue.status}&limit=${pageSize}&page=${currentPage}&date=${moment(new Date()).format("YYYY-MM-DD")}`, {
        headers: {
